...
export class FileUploaderService {
  ...
  //private API_ENDPOINT ='http://localhost/AMW_amit/cemetery-api/';
  ...
  private url= this.API_ENDPOINT + `user/uploadImage`;
  //private url= this.API_ENDPOINT + `user/uploadCSV`;
  ...
  // public functions
  public addToQueue(data: any , userid) {
    // add file to the queue
    _.each(data, (file: any) => this._addToQueue(file, this.cemid,userid));
  }
  ...
  public uploadAll(userid) {
    // upload all except already successfull or in progress
    _.each(this._files, (queueObj: FileQueueObject) => {
      if (queueObj.isUploadable()) {
        this._upload(queueObj,this.cemid,userid);
      }
    });
  }

  // private functions
  private _addToQueue(file: any,cemid,userid) {
    
    const queueObj = new FileQueueObject(file);
    // set the individual object events
    queueObj.upload = () => this._upload(queueObj,cemid,userid);
  ...
  private _upload(queueObj: FileQueueObject , cemid , userid) {
    // create form data for file
    const form = new FormData();
    form.append('file', queueObj.file, queueObj.file.name);
    form.append('cemeteryid', cemid);
